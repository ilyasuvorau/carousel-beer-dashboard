...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (sourceType === 'pdf') {
      ...
      content = result.type === 'pdf_base64' ? { type: 'pdf_base64', data: result.data } : result.content
    ...
    if (items.length > 0) {
      const { error: itemsError } = await supabase.from('price_items').insert(
        items.map(item => ({ ...item, price_list_id: priceListId, currency: item.currency ?? 'RUB' }))
      )
