...
export class UnsupportedContentTypeError extends LLMExtractionError {
	constructor(provider: string) {
		super(
			`Provider "${provider}" does not support PDF base64 content. Use plain text instead.`,
		);
...
function parseAndValidate(raw: string): ExtractedItem[] {
	...
	try {
		...
		throw new LLMExtractionError(
			`Failed to parse LLM response as JSON: ${String(err)}`,
			err,
		);
	}
 ...
 */
export async function extractPriceItems(
	content: ExtractionContent,
): Promise<ExtractedItem[]> {
